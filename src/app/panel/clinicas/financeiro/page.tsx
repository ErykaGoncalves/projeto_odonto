...
export default function MultipleSelect(): JSX.Element {
  ...
  const [sessionData, setSessionData] = useState<SessionDataItem[] | null>(
    null
  );
  ...
  useEffect(() => {
    const fetchData = async (): Promise<void> => {
      try {
        ...
        setSnackBarMessage(
          "Houve um erro ao mostrar as clínicas: " + String(error)
        );
  ...
  const handleHistoricoContent = async (): Promise<void> => {
    try {
      ...
      setSnackBarColor("error");
      setSnackBarMessage(
        "Houve um erro ao mostrar o seu histórico: " + String(error)
      );
    }
  };
  ...
  return (
    <>
      ...
      <Box sx={{ padding: "20px" }}>
        ...
        <Typography sx={{ mb: 2 }}>
          Por favor, informe a clínica ao qual deseja ver seus dados
          financeiros.
        </Typography>
                ...
                >
                  {sessionData.map((item: SessionDataItem, index: number) => (
                    ...
                    >
                      ...
                      <Typography variant="body1">
                        Total de atendimento: {item.total_atendimentos}
                      </Typography>
                      <Typography variant="body1">
                        Consultas realizadas: {item.consultas_realizadas}
                      </Typography>
                      <Typography variant="body1">
                        Consultas não realizadas:{" "}
                        {item.consultas_nao_realizadas}
                      </Typography>
                      <Typography variant="body1">
                        Valor da Consulta: {item.valor_consulta}
                      </Typography>
                      <Typography variant="body1">
                        Total do valor de consultas:{" "}
                        {item.total_valor_consultas_pagas}
                      </Typography>
                    </Box>
                  ))}
                </Box>
